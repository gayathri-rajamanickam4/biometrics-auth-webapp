...
const verifySafetynetWebauthn = require('./verify.safetynet.webauthn');
/**
...
let verifyAuthenticatorAttestationResponse = (webAuthnResponse) => {
    ...
    console.log('verifyAuthenticatorAttestationResponse::webAuthnResponse.response::', webAuthnResponse.response);

    let attestationBuffer = base64url.toBuffer(webAuthnResponse.response.attestationObject);
    let ctapMakeCredResp = cbor.decodeAllSync(attestationBuffer)[0];

    console.log('verifyAuthenticatorAttestationResponse::ctapMakeCredResp::', ctapMakeCredResp);

    let response = { verified: false };
    if (ctapMakeCredResp.fmt === 'android-safetynet') {
        let authrDataStruct = parseMakeCredAuthData(ctapMakeCredResp.authData);

        console.log('verifyAuthenticatorAttestationResponse::authrDataStruct::', authrDataStruct);

        let publicKey = COSEECDHAtoPKCS(authrDataStruct.COSEPublicKey);

        response.verified = verifySafetynetWebauthn.verifySafetyNetAttestation(webAuthnResponse);

        console.log('verifyAuthenticatorAttestationResponse::response.verified ::', response.verified);
...
let verifyAuthenticatorAssertionResponse = (webAuthnResponse, authenticators) => {
    console.log('verifyAuthenticatorAssertionResponse::webAuthnResponse.response::', webAuthnResponse.response);

    console.log('verifyAuthenticatorAssertionResponse::authenticator::', authenticators);
    ...
    let response = { verified: false };
    console.log('authr.fmt::', authr.fmt);
    if (authr.fmt === 'android-safetynet') {
        let authrDataStruct = parseGetAssertAuthData(authenticatorData);

        console.log('verifyAuthenticatorAssertionResponse::authrDataStruct::', authrDataStruct);
        ...
        console.log('verifyAuthenticatorAssertionResponse::response.verified', response.verified);
